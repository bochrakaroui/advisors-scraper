...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, help="Processed CSV path. Defaults to a date folder inside ./UBS.")
...
def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "ubs_selected_fields.csv"
...
def extract_file_date(input_path: Path) -> str:
    ...
    for pattern in ("%Y%m%d_%H%M%S", "%d%m%Y_%H%M%S"):
        ...
        try:
            return datetime.strptime(timestamp, pattern).strftime("%d/%m/%Y")
        except ValueError:
            continue
