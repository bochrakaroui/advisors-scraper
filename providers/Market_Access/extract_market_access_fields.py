"""Extract selected ETF fields from the latest Market Access snapshot."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from a Market Access ETF snapshot."
        )
    )
    parser.add_argument(
        "--input",
        type=Path,
        help="Market Access snapshot JSON path. Defaults to the latest market_access_etf_export.json.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="CSV output path. Defaults to the same folder as the source market_access_etf_export.json file.",
    )
...
def find_latest_download(input_dir: Path) -> Path:
    candidates = sorted(
        (path for path in input_dir.rglob("market_access_etf_export.json") if path.is_file()),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError(f"No market_access_etf_export.json files found in {input_dir}")
    return candidates[0]


def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "market_access_selected_fields.csv"
...
def parse_snapshot(path: Path) -> tuple[str, list[dict[str, object]]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(
            f"Unexpected Market Access snapshot in {path}: expected listing_rows to be a list."
        )
...
def transform_row(source_row: dict[str, object], scrape_date: str) -> dict[str, str]:
    row_date = parse_iso_date(source_row.get("date")) or parse_iso_date(source_row.get("nav_date")) or scrape_date
    return {
...
def extract_rows(input_path: Path | None = None) -> list[dict[str, str]]:
    ...
    named_rows = [row for row in source_rows if clean_text(row.get("etf_name"))]
    return dedupe_rows([transform_row(row, scrape_date) for row in named_rows])
