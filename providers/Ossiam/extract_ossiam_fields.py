"""Extract the selected ETF fields from the latest downloaded Ossiam snapshot."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract selected ETF fields from an Ossiam .json snapshot."
    )
    parser.add_argument(
        "--input",
        type=Path,
        help="Downloaded Ossiam .json snapshot. Defaults to the latest ossiam_export.json.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Output CSV path. Defaults to ossiam_selected_fields.csv next to the input JSON.",
    )
...
def find_latest_download(input_dir: Path) -> Path:
    candidates = sorted(
        (
            path
            for path in input_dir.rglob("ossiam_export.json")
            if path.is_file()
        ),
    ...
    if not candidates:
        raise FileNotFoundError(f"No ossiam_export.json files found in {input_dir}")

    return candidates[0]


def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "ossiam_selected_fields.csv"
...
def normalize_date(raw_value: str, fallback: str = "") -> str:
    ...
    # Handles ISO timestamps like "2026-07-01T08:58:28.715027"
...
def parse_snapshot(path: Path) -> tuple[list[dict[str, object]], str]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    rows = payload.get("listing_rows", [])

    if not isinstance(rows, list):
        raise ValueError(f"Unexpected Ossiam snapshot in {path}: expected listing_rows to be a list.")
...
def transform_row(source_row: dict[str, object], file_date: str) -> dict[str, str]:
    ...
    return {
        "ETF Name": clean_text(source_row.get("fund_name")),
        ...
        "AUM CCY": clean_text(source_row.get("aum_ccy")).upper() or ccy,
        "Date": file_date,
    }
...
def extract_rows(input_path: Path | None = None) -> list[dict[str, str]]:
    ...
    source_rows, captured_at = parse_snapshot(resolved_input_path)
    file_date = normalize_date(captured_at, fallback=extract_file_date(resolved_input_path))
