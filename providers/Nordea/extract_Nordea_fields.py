"""Extract selected Nordea ETF fields from the latest scraper snapshot."""
...
FALLBACK_FILENAME = "nordea_selected_fields.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract ETF Name, Issuer, ISIN, CCY, TER(bps), AUM(M), AUM CCY, and Date from Nordea ETF snapshot."
    )
    parser.add_argument("--input", type=Path, help="Nordea raw snapshot path. Defaults to latest.")
...
def transform_row(source_row: dict[str, object]) -> dict[str, str] | None:
    ...
    return {
        ...
        "AUM CCY": normalize_ccy(source_row.get("AUM CCY")) or normalize_ccy(source_row.get("CCY")),
