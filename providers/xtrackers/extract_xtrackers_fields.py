...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, help="Processed CSV path. Defaults to a date folder inside ./xtrackers.")
    parser.add_argument(
        "--enrich-tickers",
        action="store_true",
...
def build_output_path(output_dir: Path) -> Path:
    return build_run_output_dir(output_dir) / "xtrackers_selected_fields.csv"
...
def transform_row(source_row: dict[str, str], file_date: str) -> dict[str, str]:
    return {
        ...
        "TER(bps)": format_ter(source_row.get(SOURCE_COLUMNS["ter_percent"])),
