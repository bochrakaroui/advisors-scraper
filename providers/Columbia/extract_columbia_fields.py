"""Extract selected Columbia Threadneedle ETF fields from the latest snapshot."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from a Columbia Threadneedle snapshot."
        )
    )
    parser.add_argument(
        "--input",
        type=Path,
        help="Columbia snapshot JSON path. Defaults to the latest columbia_etf_export.json.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def find_latest_download(input_dir: Path) -> Path:
    candidates = sorted(
        (path for path in input_dir.rglob("columbia_etf_export.json") if path.is_file()),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError(f"No columbia_etf_export.json files found in {input_dir}")
    return candidates[0]


def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "columbia_selected_fields.csv"
...
def parse_snapshot(path: Path) -> tuple[str, list[dict[str, object]]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(
            f"Unexpected Columbia snapshot in {path}: expected listing_rows to be a list."
        )
...
def transform_row(source_row: dict[str, object], scrape_date: str) -> dict[str, str]:
    row_date = (
        ...
        or parse_iso_date(source_row.get("share_class_launch_date"))
        or scrape_date
    )
    return {
        ...
        "CCY": clean_text(source_row.get("share_class_currency") or source_row.get("fund_base_currency")).upper(),
        "TER(bps)": clean_text(source_row.get("ter_bps")),
        "AUM(M)": clean_text(source_row.get("aum_mn")),
        "AUM CCY": clean_text(source_row.get("aum_ccy") or source_row.get("fund_base_currency")).upper(),
        "Date": row_date,
    }
