"""Extract selected PIMCO ETF fields from the latest snapshot."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from a PIMCO ETF snapshot."
        )
    )
    parser.add_argument(
        "--input",
        type=Path,
        help="PIMCO snapshot JSON path. Defaults to the latest pimco_etf_export.json.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="CSV output path. Defaults to the same folder as the source pimco_etf_export.json file.",
    )
...
def find_latest_download(input_dir: Path) -> Path:
    candidates = sorted(
        (path for path in input_dir.rglob("pimco_etf_export.json") if path.is_file()),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError(f"No pimco_etf_export.json files found in {input_dir}")
    return candidates[0]


def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "pimco_selected_fields.csv"
...
def parse_snapshot(path: Path) -> tuple[str, list[dict[str, object]]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(f"Unexpected PIMCO snapshot in {path}: expected listing_rows to be a list.")
...
def transform_row(source_row: dict[str, object], scrape_date: str) -> dict[str, str]:
    row_date = parse_iso_date(source_row.get("aum_date")) or parse_iso_date(source_row.get("daily_nav_date")) or scrape_date
    return {
