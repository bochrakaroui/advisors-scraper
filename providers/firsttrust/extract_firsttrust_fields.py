"""Extract the selected ETF fields from the latest downloaded First Trust snapshot."""
...
INPUT_DIR = BASE_DIR / "firsttrust_downloads"
OUTPUT_DIR = BASE_DIR / "firsttrust_processed"
ISSUER = "First Trust"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract the selected ETF fields from a downloaded First Trust .json snapshot.")
    parser.add_argument("--input", type=Path, help="Downloaded First Trust .json snapshot. Defaults to the latest file.")
    parser.add_argument("--output", type=Path, help="Processed CSV path. Defaults to ./firsttrust_processed.")
...
def build_output_path(output_dir: Path) -> Path:
    ...
    return output_dir / f"firsttrust_selected_fields_{timestamp}.csv"
...
def parse_snapshot_rows(path: Path) -> list[dict[str, str]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(f"Unexpected First Trust snapshot in {path}: expected a list of rows.")
    return rows
