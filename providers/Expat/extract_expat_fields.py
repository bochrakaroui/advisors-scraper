"""Extract the selected ETF fields from the latest downloaded Expat snapshot."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract selected ETF fields from an Expat .json snapshot."
    )
    parser.add_argument(
        "--input",
        type=Path,
        help="Downloaded Expat .json snapshot. Defaults to the latest expat_export.json.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help="Output CSV path. Defaults to expat_selected_fields.csv next to the input JSON.",
    )
...
def find_latest_download(input_dir: Path) -> Path:
    candidates = sorted(
        (
            path
            for path in input_dir.rglob("expat_export.json")
            if path.is_file()
        ),
    ...
    if not candidates:
        raise FileNotFoundError(f"No expat_export.json files found in {input_dir}")

    return candidates[0]


def build_output_path(input_path: Path) -> Path:
    return input_path.parent / "expat_selected_fields.csv"
...
def parse_snapshot_rows(path: Path) -> list[dict[str, object]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(f"Unexpected Expat snapshot in {path}: expected listing_rows to be a list.")
